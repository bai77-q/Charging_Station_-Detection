            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    if save_flag:
                        if "不正常" in s:
                            print("检测到不正常充电柱...")
                            get_images(p.name, "不正常充电", count_charge, im0)
                            count_charge += 1
                            save_flag = False

                        elif "小广告" in s:
                            print("检测到小广告...")
                            get_images(p.name, "小广告", count_ad, im0)
                            count_ad += 1
                            save_flag = False

                        elif "人" in s and "灭火器" in s:
                            print("检测到人使用灭火器...")
                            get_images(p.name, "灭火器", count_fire, im0)
                            count_fire += 1
                            save_flag = False
                    frame_count += 1
...
def parse_opt(images_path):
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'charge_station/data_add_dataset.yaml',
...
import moviepy.editor as mp
import gradio as gr
# from llm_station_detection import get_gpt4_instruction  # 确保这个函数是定义好的
...
def auto_process_folder(task):
    ...
    for file_name in os.listdir(save_dir):
        ...
        if allowed_file(file_name):
            if file_name.endswith("png") or file_name.endswith("jpg") or file_name.endswith("jpeg"):
                image = Image.open(file_path)
                instruction = process_image(image, task)
                results.append((file_name, instruction))
            # 对于视频文件可以添加其他处理方式
    return results
...
def list_processed_files():
    script_dir = os.path.dirname(os.path.abspath(__file__))
    processed_dir = os.path.join(script_dir, 'processed_files')  # 假设处理后的文件存放在该目录
    if not os.path.exists(processed_dir):
        os.makedirs(processed_dir)
    return [f for f in os.listdir(processed_dir) if allowed_file(f)]


def get_gpt4_output(selected_file, task):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(script_dir, 'processed_files', selected_file)
...
with gr.Blocks() as interface:
    with gr.Row():
        ...
        with gr.Column():
            process_button = gr.Button("扫描并处理文件夹内的新数据")
            results_output = gr.JSON(label="检查结果")
    with gr.Row():
        with gr.Column():
            check_processed_button = gr.Button("检查处理后的文件")
            processed_files_dropdown = gr.Dropdown(label="选择处理后的文件", choices=[])
    ...
    process_button.click(auto_process_folder, inputs=task_dropdown, outputs=results_output)
    check_processed_button.click(list_processed_files, outputs=processed_files_dropdown)
    gpt4_output_button.click(get_gpt4_output, inputs=[processed_files_dropdown, task_dropdown],
                             outputs=gpt4_output_textbox)
