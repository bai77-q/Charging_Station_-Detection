...
def parse_opt(images_path):
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'charge_station/data_add_dataset.yaml', help='(optional) dataset.yaml path')
...
def save_file(file):
    ...
    # 获取脚本所在的绝对路径
    script_dir = os.path.dirname(os.path.abspath(__file__))
    save_dir = os.path.join(script_dir, 'test_files')
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
